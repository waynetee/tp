...
/**
 * Changes the remark of an existing property in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the property identified "
            + "by the index number used in the last property listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the property in the filtered property list to edit the remark
     * @param remark of the property to be updated to
     */
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of property data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the property’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a property.

### Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.property`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each property.

Simply add the following to [`seedu.address.ui.PropertyCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
``` java
public PersonCard(Person property, int displayedIndex) {
    //...
    remark.setText(property.getRemark().value);
}
```
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person propertyToEdit = lastShownList.get(index.getZeroBased());
        Person editedProperty = new Person(
                propertyToEdit.getName(), propertyToEdit.getPhone(), propertyToEdit.getEmail(),
                propertyToEdit.getAddress(), remark, propertyToEdit.getTags());

        model.setPerson(propertyToEdit, editedProperty);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedProperty));
    }
     ...
     * {@code propertyToEdit}.
     */
    private String generateSuccessMessage(Person propertyToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, propertyToEdit);
    }
```
