...
1. Now, step over until you read the statement that calls the `execute()` method of the `EditCommand` object received, and step into that `execute()` method (partial code given below):
   ...
   **`EditCommand#execute()`:**
   ...
   public CommandResult execute(Model model) throws CommandException {
       ...
       Person editedProperty = createEditedPerson(propertyToEdit, editPersonDescriptor);
...
1. As suspected, `command#execute()` does indeed make changes to the `model` object. Specifically,
   * it uses the `setPerson()` method (defined in the interface `Model` and implemented in `ModelManager` as per the usual pattern) to update the property data.
   * it uses the `updateFilteredPersonList` method to ask the `Model` to populate the 'filtered list' with _all_ properties.<br>
     FYI, The 'filtered list' is the list of properties resulting from the most recent operation that will be shown to the user immediately after. For the `edit` command, we populate it with all the properties so that the user can see the edited property along with all other properties. If this was a `find` command, we would be setting that list to contain the search results instead.<br>
     To provide some context, given below is the class diagram of the `Model` component. See if you can figure out where the 'filtered list' of properties is being tracked.
